...
k.set({ "n" }, "<leader>f", picker.files, { desc = "Find files" })
k.set({ "n" }, "<leader>r", picker.git_grep, { desc = "Grep" })
k.set({ "n" }, "<leader>a", picker.buffers, { desc = "Opened buffer" })
k.set({ "n" }, "<leader>M", picker.man, { desc = "Manpages" })
k.set({ "n" }, "<leader>H", picker.help, { desc = "Help pages" })
...
local find_files_current = function()
	local cwd = sanitize_path(vim.loop.cwd())
	picker.files({ cwd = cwd})
end

local git_files_current = function()
	local cwd = sanitize_path(vim.loop.cwd())
	picker.git_files({ cwd = cwd })
end
